...
def build_rescue_net(code, cfg: Dict, var_dim: int = 16, check_dim: int = 6, global_dim: int = 9, candidate_dim: int = 14):
    """Build the v14 residual AI/Tanner-GRAND rescue network.

    v14.3 note: TensorFlow mixed precision can auto-cast Keras layer outputs to
    bfloat16 while the LDPC parity matrix constants remain float32. TensorFlow's
    Einsum op requires matching input dtypes, so all Tanner message-passing
    tensors are explicitly kept in float32. This prevents the v14.2 H100 crash:
        expected a float tensor but is a bfloat16 tensor.
    """
    import tensorflow as tf

    class CandidateReranker(tf.keras.layers.Layer):
        def __init__(self, hidden_dim: int, dropout: float):
            super().__init__(name="candidate_reranker_v14")
        ...
        def call(self, packet_embedding, candidate_features, training: bool = False):
            packet_embedding = tf.cast(packet_embedding, tf.float32)
            candidate_features = tf.cast(candidate_features, tf.float32)
            cnum = tf.shape(candidate_features)[1]
            pkt = tf.tile(packet_embedding[:, None, :], [1, cnum, 1])
            x = tf.concat([pkt, candidate_features], axis=-1)
            x = self.d1(x)
    ...
    class RescueNet(tf.keras.Model):
        def __init__(self):
            super().__init__(name="tf_rescue_net_v14_3")
            mcfg = cfg.get("model", {})
            tcfg = cfg.get("train", {})
        ...
        def call(self, inputs: Dict[str, object], training: bool = False):
            ...
            # Dense layers can emit bfloat16 under mixed precision; cast their
            # outputs back to float32 before every Tanner graph einsum.
            v = tf.cast(self.v_in(var_features), tf.float32)
            c = tf.cast(self.c_in(check_features), tf.float32)
            h = tf.cast(self.h_mat, tf.float32)
            ...
            for vu, cu, vn, cn, drop in zip(self.v_updates, self.c_updates, self.v_norms, self.c_norms, self.dropouts):
                ...
                v_delta = vu(tf.concat([v, tf.cast(c_to_v, tf.float32)], axis=-1), training=training)
                v_delta = tf.cast(drop(v_delta, training=training), tf.float32)
                v = tf.cast(vn(v + v_delta), tf.float32)

                c_delta = cu(tf.concat([c, tf.cast(v_to_c, tf.float32)], axis=-1), training=training)
                c_delta = tf.cast(drop(c_delta, training=training), tf.float32)
                c = tf.cast(cn(c + c_delta), tf.float32)
            ...
            g = tf.cast(self.g_in(global_features), tf.float32)

            packet = tf.cast(self.packet_norm(self.packet_d1(tf.concat([
                mean_pool + g,
                max_pool,
                token_pool,
                check_mean_pool,
            ...
            out = {
                ...
                "segment_logits": tf.cast(self.segment_head(packet_f32), tf.float32),
                "weight_logits": tf.cast(self.weight_head(packet_f32), tf.float32),
                "standard_logits": tf.cast(tf.squeeze(self.standard_head(packet_f32), axis=-1), tf.float32),
                "expanded_logits": tf.cast(tf.squeeze(self.expanded_head(packet_f32), axis=-1), tf.float32),
                "rescue_logits": tf.cast(tf.squeeze(self.rescue_head(packet_f32), axis=-1), tf.float32),
